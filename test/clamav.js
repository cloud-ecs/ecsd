...
describe('clamav', () => {
  describe('clamdscan', () => {
    let avail
    before(async () => {
      avail = await clamav.binAvailable().catch(() => false)
    })
    ...
    it('detects eicar virus', async (t) => {
      ...
      assert.match(results.fail[0], /Eicar/i)
    })
  ...
  describe('TCP', () => {
    let avail
    before(async () => {
      avail = await clamav.tcpListening().catch(() => false)
    })
  ...
  describe('unix socket', () => {
    let avail
    before(async () => {
      avail = await clamav.socketFound().catch(() => false)
    })
  ...
  describe('scan dispatch', () => {
    let avail
    before(async () => {
      avail = await clamav.isAvailable().catch(() => false)
    })
