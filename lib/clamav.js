...
import ClamStream from './clamav-stream.js'
...
class Scanner extends BaseScanner {
  ...
  scanTcp(file) {
    ...
    return this.scanStream(this.cfg.net, file)
  }

  scanSocket(file) {
    if (!file) return Promise.reject(new Error('file is required!'))
    if (!this.found.socket) {
      return Promise.reject(new Error('unix socket not found'))
    }

    return this.scanStream(this.found.socket, file)
  }

  // INSTREAM ships the message body to clamd instead of a path, so the daemon
  // never needs filesystem access to the file (it runs as a user that often
  // can't read the caller's working tree).
  scanStream(connectOpts, file) {
