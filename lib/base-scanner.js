...
import { findBin } from './detect-cli.js'
import { findTcp } from './detect-tcp.js'
import { findSocket } from './detect-socket.js'
...
class Scanner {
  ...
  async scan(file, envelope = []) {
    switch (this.available.any) {
      case 'tcp':
        return this.scanTcp(file, envelope)
      case 'socket':
        return this.scanSocket(file, envelope)
      case 'cli':
  ...
  async scanSocket(_file, _envelope) {
