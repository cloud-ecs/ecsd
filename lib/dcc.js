...
class Scanner extends BaseScanner {
  constructor(name) {
    ...
    // FreeBSD ports/pkg installs the dccifd listener socket here; override in
    // config on other systems (findSocket also searches /tmp and /var/run).
    if (!this.cfg.socket) this.cfg.socket = '/usr/local/libexec/dccifd'
  ...
  // HELO, sender, and recipient(s), terminated by a blank line. ecsd scans a
  // bare file with no SMTP context, so the envelope is a placeholder; DCC's
  // bulk detection keys off the body checksums, not the envelope.
  requestPreamble() {
    const envelope = ['header', '127.0.0.1', 'localhost', '', 'postmaster']
